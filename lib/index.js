 ...
 */
exports.default = async ([find, replace], config) => {
    ...
    try {
        ...
        const groupedByRepo = outcomes.reduce((collection, item) => {
 ...
 */
async function processResults(results, find, replace) {
    return results.reduce(async (queue, item) => {
        try {
            const collection = await queue;
 ...
 */
async function queryMatches(match, find, replace) {
 ...
 */
async function replaceBlobs(repo, replacements, find, replace) {
 ...
 */
async function updateBranch(blobItem, branchName, repo) {
